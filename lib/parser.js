...
function parseSgr(chunk) {
  ...
  // If we get here we are a good sgi
    ...
    if ((args[0] === 38) && (args[1] === 2)) {
      sgr = { type: 'sgr' , cmd: 'fg-rgb' , args: [ args[2], args[3] , args[4] ] };
      return sgr;
    }

    if ((args[0] === 38) && (args[1] === 5)) {
      sgr = { type: 'sgr' , cmd: 'fg-256', args: [ args[2] ] };
      return sgr;
    }

    if ((args[0] === 48) && (args[1] === 2)) {
      sgr = { type: 'sgr' , cmd: 'bg-rgb' , args: [ args[2], args[3] , args[4] ] };
      return sgr;
    }

    if ((args[0] === 48) && (args[1] === 5)) {
      sgr = { type: 'sgr' , cmd: 'bg-256' , args: [ args[2] ] };
      return sgr;
    }
    ...
    if (args.length === 1) {
      return { type: 'sgr' , cmd: args[0], args: args };
    }
