...
function parseSgr(args , handler) {
  ...
  if ((args[0] === 38) && (args[1] === 2)) {
    sgr = { type: 'sgr' , cmd: 'fg-rgb' , rgb: [ args[2], args[3] , args[4] ] };
    handler(sgr);
    return;
  }

  if ((args[0] === 38) && (args[1] === 5)) {
    sgr = { type: 'sgr' , cmd: 'fg-'+args[2] };
    handler(sgr);
    return;
  }

  if ((args[0] === 48) && (args[1] === 2)) {
    sgr = { type: 'sgr' , cmd: 'bg-rgb' , rgb: [ args[2], args[3] , args[4] ] };
    handler(sgr);
    return;
  }

  if ((args[0] === 48) && (args[1] === 5)) {
    sgr = { type: 'sgr' , cmd: 'bg-'+args[2] };
    handler(sgr);
    return;
  }

  args.forEach(function(a) {
    sgr = { type: 'sgr' , cmd: a, args: args };
    handler(sgr);
  });
