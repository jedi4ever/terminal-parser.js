...
var csi = {
  ...
  'A': function(p) { return op([ [ 'cursor_move', 0 , -(p.args[0] || 1) ] ])(); },

  // CSI P s B
  // Cursor Down P s Times (default = 1) (CUD)
  'B': function(p) { return op([ [ 'cursor_move' , 0, (p.args[0] || 1)  ] ])(); },

  // CSI P s C
  // Cursor Forward P s Times (default = 1) (CUF)
  'C': function(p) { return op([ [ 'cursor_move', (p.args[0] || 1) , 0 ] ])(); },

  // CSI P s D
  // Cursor Backward P s Times (default = 1) (CUB).
  'D': function(p) { return op([ [ 'cursor_move', -(p.args[0] || 1) , 0 ] ])(); },

  // CSI P s E
  // Cursor Next Line P s Times (default = 1) (CNL).
  'E': function(p) { return op([ [ 'cursor_move', 0 , (p.args[0] ||1) ] , [ 'cursor_set', 0, null ] ])(); },
  // CSI P s F
  // Cursor Preceding Line P s Times (default = 1) (CPL)

  // CSI P s G
  // Cursor Character Absolute [column] (default = [row,1]) (CHA)
  'G': function(p) { return op([ [ 'cursor_char_absolute', p.args ] ])(); },
  ...
  'I': function(p) { return op([ [ 'cursor_tab' , (p.args[0] || 1)  ] ])(); },
  ...
  // P s = 2 → Selective Erase All.
  '?K': function(p) { return op([ [ 'DECSEL', p.args[0] ] ])(); },

  // CSI P s L
  // Insert P s Line(s) (default = 1) (IL).
  'L': function(p) { return op([ [ 'insert_lines', p.args[0] ] ])(); },

  // CSI P s M
  // Delete P s Line(s) (default = 1) (DL)
  'M': function(p) { return op([ [ 'delete_lines', ( p.args[0] || 1) ] ])(); },

  // CSI P s P
  // Delete P s Character(s) (default = 1) (DCH).
  'P': function(p) { return op([ [ 'delete_chars', ( p.args[0] || 1)] ])(); },

  // CSI P s S
  // Scroll up P s lines (default = 1) (SU).
  'S': function(p) { return op([ [ 'scroll', ( p.args[0] || 1) ] ])(); },

  // CSI P s T
  // Scroll down P s lines (default = 1) (SD).
  'T': function(p) { return op([ [ 'scroll', - (p.args[0] || 1) ] ])(); },

  // CSI P s X
  // Erase P s Character(s) (default = 1) (ECH).
  'X': function(p) { return op([ [ 'erase_char', - (p.args[0] || 1) ] ])(); },

  // CSI P s Z
  // Cursor Backward Tabulation P s tab stops (default = 1) (CBT).
  'Z': function(p) { return op([ [ 'cursor_tab' , - (p.args[0] || 1) ] ])(); },

  // CSI P m `
  // Character Position Absolute [column] (default = [row,1]) (HPA).
  '`': function(p) { return op([ [ 'char_position' , (p.args[0] || 1) ] ])(); },
...
var esc = {
  ...
  'M': op( [ [ 'reverse_index' ] ]), // Reverse Index ( RI is 0x8d).
  'D': op( [ [ 'index' ] ]) // Index ( IND is 0x84).
};

var sgr = {
  '0': function(p) { return op( [ [ 'ATTR', 'RESET' ] ] )(); },
  '1': op( [ [ 'ATTR', 'BOLD', 'ON' ] ] ),
  '3': op( [ [ 'ATTR', 'ITALIC', 'ON' ] ] ),
  ...
  'fg-256': function(p) { return op([ [ 'ATTR', 'FG', (+p.args[0]) ] ])(); },
  'bg-256': function(p) { return op([ [ 'ATTR', 'BG', (+p.args[0]) ] ])(); },
};
