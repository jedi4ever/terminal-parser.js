...
function emitKey(stream, s) {
  ...
  key.sequence = s;
  var metaKeyCodeParts = metaKeyCodeRe.exec(s);
  var functionKeyCodeParts = functionKeyCodeRe.exec(s);
  ...
  } else if (metaKeyCodeParts) {
    // meta+character key
    key.name = metaKeyCodeParts[1].toLowerCase();
    key.meta = true;
    key.shift = /^[A-Z]$/.test(metaKeyCodeParts[1]);

  } else if (functionKeyCodeParts) {
    // ansi escape sequence

    parts = functionKeyCodeParts;
  ...
  if (key.code == '[M') {
    ...
    var b = seq.charCodeAt(3);
    key.x = seq.charCodeAt(4) - 32;
    key.y = seq.charCodeAt(5) - 32;
