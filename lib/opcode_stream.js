...
function OpcodeStream(buffer) {
  OpcodeStream.super_.call(this, { decodeStrings: false , objectMode: true });
}

util.inherits(OpcodeStream, stream.Transform);
module.exports = OpcodeStream;

OpcodeStream.prototype._transform = function(chunk, encoding, callback) {
  try {
    var p = chunk;
    var key = [ p.code , p.cmd ].join('-');
    var t = transformations[p.code];
    if (t) {
      var tcmd = t[p.cmd];
      if (tcmd) {
        this.push(tcmd(p));
